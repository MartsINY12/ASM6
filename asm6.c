...
#define WORDMAX 512     //used with getword()
...
int pass=0;
int minpasses=0;//min # of tries before giving up
int maxpasses=7;//# of tries before giving up
...
int differentResult=1;//will be true until 2 tries in a row are the same
...
int main(int argc,char **argv) {
        ...
        if(*argv[i]=='-' || *argv[i]=='/') {
            switch(argv[i][1]) {
                ...
                case 'q':
                    ...
                    break;
				case 'm':
					minpasses=atoi(argv[i+1]);
					i++;
					break;
				case 'r':
					maxpasses=atoi(argv[i+1]);
					i++;
					break;
                default:
    ...
    do {
        pass++;
        if((pass==maxpasses || (p==lastlabel)) && (pass>minpasses) && differentResult)
		...
		// Is it different from previous output file?
		if (pass>1)
		{
